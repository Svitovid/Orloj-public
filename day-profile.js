(function(root,factory){
  ...
  function addMonthsDateKey(key,amount){var p=parseDateKey(key);if(!p)return null;var targetMonth=p.month-1+Number(amount||0),year=p.year+Math.floor(targetMonth/12),month=((targetMonth%12)+12)%12,maxDay=new Date(Date.UTC(year,month+1,0,12)).getUTCDate(),day=Math.min(p.day,maxDay);return year+"-"+pad(month+1)+"-"+pad(day);}
  function rangeBounds(startKey,endKey,tz){if(!parseDateKey(startKey)||!parseDateKey(endKey)||!validTimeZone(tz))throw new Error("Neplatný rozsah nebo časové pásmo.");var start=zonedLocalToUtc(startKey,"00:00",tz),end=zonedLocalToUtc(endKey,"00:00",tz);if(end<=start)throw new Error("Konec rozsahu musí následovat po začátku.");return{start:start,end:end,dateKey:startKey,endKey:endKey,timezone:tz,days:(end-start)/DAY};}
  function phaseEventsRange(A,bounds){var defs=[[0,"Nov","☉☽"],[90,"První čtvrť","◐"],[180,"Úplněk","☽"],[270,"Poslední čtvrť","◑"]],out=[];defs.forEach(function(def){var cursor=bounds.start-60000,guard=0;while(cursor<bounds.end&&guard++<48){var found=A.SearchMoonPhase(def[0],new Date(cursor),(bounds.end-cursor)/DAY+2);if(!found)break;var at=found.date.getTime();if(at>=bounds.end)break;if(at>=bounds.start)out.push({kind:"phase",at:at,title:def[1],glyph:def[2],detail:"Přesná elongace Slunce a Měsíce · "+def[0]+"°",layer:"astronomie",phaseAngle:def[0]});cursor=at+60000;}});return out;}
  function eclipseEventsRange(A,bounds){var out=[],solarNames={total:"Úplné zatmění Slunce",annular:"Prstencové zatmění Slunce",partial:"Částečné zatmění Slunce",hybrid:"Hybridní zatmění Slunce"},lunarNames={total:"Úplné zatmění Měsíce",partial:"Částečné zatmění Měsíce",penumbral:"Polostínové zatmění Měsíce"};function collect(search,isSolar){var cursor=bounds.start,guard=0;while(cursor<bounds.end&&guard++<12){var found=search(new Date(cursor));if(!found||!found.peak)break;var at=found.peak.date.getTime();if(at>=bounds.end)break;if(at>=bounds.start)out.push({kind:"eclipse",at:at,title:(isSolar?solarNames[found.kind]:lunarNames[found.kind])||(isSolar?"Zatmění Slunce":"Zatmění Měsíce"),glyph:isSolar?"☉":"☽",detail:"Maximum "+(isSolar?"globálního průběhu":"zatmění")+" · zakrytí "+Math.round((found.obscuration==null?(isSolar?1:0):found.obscuration)*100)+" % · místní viditelnost se liší",layer:"astronomie",eclipseType:isSolar?"solar":"lunar",classification:found.kind});cursor=at+20*DAY;}}collect(function(date){return A.SearchGlobalSolarEclipse(date);},true);collect(function(date){return A.SearchLunarEclipse(date);},false);return out;}
  function seasonEventsRange(A,bounds,startKey,endKey){var first=parseDateKey(startKey),last=parseDateKey(endKey),out=[],seen={},names=[["mar_equinox","Jarní rovnodennost"],["jun_solstice","Letní slunovrat"],["sep_equinox","Podzimní rovnodennost"],["dec_solstice","Zimní slunovrat"]];for(var year=first.year-1;year<=last.year+1;year++){var seasons=A.Seasons(year);names.forEach(function(def){var event=seasons[def[0]],at=event&&event.date.getTime();if(at!=null&&within(at,bounds)&&!seen[at]){seen[at]=true;out.push({kind:"season",at:at,title:def[1],glyph:"☉",detail:"Slunce překračuje hlavní bod tropického roku.",layer:"astronomie"});}});}return out;}
  function aspectError(A,a,b,def,at){return Math.abs(angularSeparation(longitudeById(A,a,new Date(at)),longitudeById(A,b,new Date(at)))-def.angle);}
  function refineBoundary(fn,limit,lo,hi){var activeLo=fn(lo)<=limit;for(var i=0;i<34;i++){var mid=(lo+hi)/2,activeMid=fn(mid)<=limit;if(activeMid===activeLo)lo=mid;else hi=mid;}return(lo+hi)/2;}
  function aspectWindows(A,bounds,options){options=options||{};var step=(options.stepHours||6)*3600000,start=bounds.start-step,end=bounds.end+step,times=[],sampleLongitudes=[];for(var at=start;at<=end;at+=step){times.push(at);var row={};BODY_META.forEach(function(meta){row[meta.id]=longitude(A,meta,new Date(at));});sampleLongitudes.push(row);}var out=[];for(var i=0;i<BODY_META.length;i++)for(var j=i+1;j<BODY_META.length;j++)ASPECTS.forEach(function(def){var values=times.map(function(at,index){return Math.abs(angularSeparation(sampleLongitudes[index][BODY_META[i].id],sampleLongitudes[index][BODY_META[j].id])-def.angle);}),segments=[],segment=null;for(var k=0;k<values.length;k++){if(values[k]<=def.orb&&!segment)segment={first:k,last:k};else if(values[k]<=def.orb)segment.last=k;else if(segment){segments.push(segment);segment=null;}}if(segment)segments.push(segment);segments.forEach(function(seg){if(times[seg.last]<bounds.start||times[seg.first]>=bounds.end)return;var fn=function(t){return aspectError(A,BODY_META[i].id,BODY_META[j].id,def,t);},from=times[seg.first],to=times[seg.last];if(seg.first>0&&values[seg.first-1]>def.orb)from=refineBoundary(fn,def.orb,times[seg.first-1],times[seg.first]);if(seg.last<times.length-1&&values[seg.last+1]>def.orb)to=refineBoundary(fn,def.orb,times[seg.last],times[seg.last+1]);var best=seg.first;for(var n=seg.first+1;n<=seg.last;n++)if(values[n]<values[best])best=n;var lo=times[Math.max(0,best-1)],hi=times[Math.min(times.length-1,best+1)],peak=refineAspectMinimum(A,BODY_META[i].id,BODY_META[j].id,def.angle,lo,hi),orb=fn(peak);if(peak<from||peak>to){peak=times[best];orb=values[best];}if(to>bounds.start&&from<bounds.end){var shownAt=Math.max(bounds.start,Math.min(peak,bounds.end-1));out.push({kind:"aspect-window",at:shownAt,peakAt:peak,start:Math.max(from,bounds.start),end:Math.min(to,bounds.end),unclippedStart:from,unclippedEnd:to,title:BODY_META[i].name+" "+def.name.toLowerCase()+" "+BODY_META[j].name,glyph:def.glyph,detail:(orb<.03?"Přesný aspekt":"Největší přiblížení")+" · "+def.angle+"° · orb "+orb.toFixed(2)+"°",layer:"astrologická gramatika",a:BODY_META[i].id,b:BODY_META[j].id,aspect:def,orb:orb,exact:orb<.03});}});});out.sort(function(a,b){return a.at-b.at;});return out;}
  function personalTransitWindows(A,bounds,profile,options){var natal=natalChart(A,profile);if(!natal)return[];options=options||{};var selectedTransits=Array.isArray(options.transitIds)?options.transitIds:null,selectedTargets=Array.isArray(options.targetIds)?options.targetIds:null,transitBodies=BODY_META.filter(function(meta){return!selectedTransits||selectedTransits.indexOf(meta.id)>=0;}),natalTargets=natal.points.filter(function(point){return!selectedTargets||selectedTargets.indexOf(point.id)>=0;}),step=(options.stepHours||6)*3600000,start=bounds.start-step,end=bounds.end+step,times=[],rows=[],limits={sun:2.5,moon:2,mercury:2.2,venus:2.2,mars:2.5,jupiter:2.5,saturn:2.5,uranus:2,neptune:2,pluto:2};for(var at=start;at<=end;at+=step){times.push(at);var row={};transitBodies.forEach(function(meta){row[meta.id]=longitude(A,meta,new Date(at));});rows.push(row);}var out=[];transitBodies.forEach(function(transit){natalTargets.forEach(function(target){ASPECTS.forEach(function(def){var limit=Math.min(def.orb,limits[transit.id]||2.5),values=times.map(function(at,index){return Math.abs(angularSeparation(rows[index][transit.id],target.lon)-def.angle);}),segment=null,segments=[];for(var k=0;k<values.length;k++){if(values[k]<=limit&&!segment)segment={first:k,last:k};else if(values[k]<=limit)segment.last=k;else if(segment){segments.push(segment);segment=null;}}if(segment)segments.push(segment);segments.forEach(function(seg){if(times[seg.last]<bounds.start||times[seg.first]>=bounds.end)return;var fn=function(t){return Math.abs(angularSeparation(longitudeById(A,transit.id,new Date(t)),target.lon)-def.angle);},from=times[seg.first],to=times[seg.last];if(seg.first>0&&values[seg.first-1]>limit)from=refineBoundary(fn,limit,times[seg.first-1],times[seg.first]);if(seg.last<times.length-1&&values[seg.last+1]>limit)to=refineBoundary(fn,limit,times[seg.last],times[seg.last+1]);var best=seg.first;for(var n=seg.first+1;n<=seg.last;n++)if(values[n]<values[best])best=n;var lo=times[Math.max(0,best-1)],hi=times[Math.min(times.length-1,best+1)];for(var q=0;q<28;q++){var x1=lo+(hi-lo)/3,x2=hi-(hi-lo)/3;if(fn(x1)<=fn(x2))hi=x2;else lo=x1;}var peak=(lo+hi)/2,orb=fn(peak);if(to>bounds.start&&from<bounds.end){var shownAt=Math.max(bounds.start,Math.min(peak,bounds.end-1));out.push({kind:"personal",at:shownAt,peakAt:peak,start:Math.max(from,bounds.start),end:Math.min(to,bounds.end),unclippedStart:from,unclippedEnd:to,title:transit.name+" "+def.name.toLowerCase()+" nativní "+target.name,glyph:def.glyph,detail:"Osobní tranzit · orb "+orb.toFixed(2)+"°",layer:"osobní",body:transit.id,target:target.id,transit:transit,natal:target,aspect:def,orb:orb,exact:orb<.03});}});});});});out.sort(function(a,b){return a.at-b.at;});return out;}
  function rangeEvents(A,startKey,endKey,tz,profile,options){var bounds=rangeBounds(startKey,endKey,tz),events=[].concat(eclipseEventsRange(A,bounds),phaseEventsRange(A,bounds),seasonEventsRange(A,bounds,startKey,endKey),ingressEvents(A,bounds),stationEvents(A,bounds),aspectWindows(A,bounds,options));events.sort(function(a,b){return a.at-b.at;});return{bounds:bounds,events:events,personal:personalTransitWindows(A,bounds,profile,options)};}
  ...
  function dayOfYear(key){var p=parseDateKey(key);if(!p)return null;return Math.floor((Date.UTC(p.year,p.month-1,p.day)-Date.UTC(p.year,0,0))/DAY);}
