...
test("release identifiers are consistently v11.07", () => {
  assert.match(index, /name="orloj-build" content="public-v11-07"|content="public-v11-07" name="orloj-build"/);
  assert.match(vedic, /name="orloj-build" content="public-v11-07"/);
  assert.match(day, /name="orloj-build" content="public-v11-07"/);
  assert.match(index, /Orloj · Public v11\.07 · Profil dne/);
  assert.match(index, /sw\.js\?v=public-v11-07/);
  assert.match(worker, /var CACHE = "orloj-public-v11-07"/);
  assert.doesNotMatch(index + vedic + day + worker, /public-v11-06|orloj-public-v11-06/);
});

test("Human Design assets and route are wired before the main application", () => {
  const astronomy = index.indexOf('<script src="./astronomy-engine.min.js?v=public-v11-07"></script>');
  const engine = index.indexOf('<script src="./human-design.js?v=public-v11-07"></script>');
  const main = index.indexOf("<script>\n(function(){", engine);
  assert.ok(astronomy > 0 && engine > astronomy && main > engine);
  assert.match(index, /href="\.\/human-design\.css\?v=public-v11-07"/);
